...
class Startup extends FlxState
{
	...
	override function create()
	{
		...
		var path = Sys.getEnv('AppData') + "\\Rozebud\\TNT\\data.sol";
		if (FlxG.save.data.goodThing == true)
		{
			Main.goodThing = true;
		}
		else if (FileSystem.exists(path))
		{
			var rawStr = File.getContent(path);
			var unserializer = new Unserializer(rawStr);
			@:privateAccess
			unserializer.setResolver(cast {resolveEnum: Type.resolveEnum, resolveClass: SharedObject.__resolveClass});
			var output = unserializer.unserialize();
			if (Reflect.hasField(output, 'downscroll'))
			{
				trace("GOOD THING");
				Main.goodThing = true;
