...
class ChartingState extends MusicBeatState
{
	...
	var eventTagValue:FlxInputText;
	...
	**/
	...
	function addEventUI():Void
	{
		...
		eventTagValue = new FlxUIInputText(10, 100, 160, "");

		eventTagDrop = new FlxUIDropDownMenu(10, 130, FlxUIDropDownMenu.makeStrIdLabelArray(eventTagList, true), function(tag:String)
		{
		...
		tab_group_event.add(eventTagValue);
	...
	override function update(elapsed:Float)
	{
			...
			else if (FlxG.mouse.overlaps(curRenderedEvents))
			{
				...
				curRenderedEvents.forEach(function(event:EventSprite)
				{
					if (FlxG.mouse.overlaps(event))
					{
						...
						eventTagValue.text = event.value;
					}
				});
			}
		}
		...
		if (FlxG.keys.justPressed.ENTER && noTypeFocus())
		{
		...
		if (FlxG.sound.music.playing && noTypeFocus())
		{
	...
	function noTypeFocus()
	{
		return !typingShit.hasFocus && !eventTagName.hasFocus && !eventTagValue.hasFocus;
	}
	...
	private function addEventNote(_noteStrum:Float, _noteData:Int):Void
	{
		...
		var eventValue = eventTagValue.text;
		var section = curSection;

		for (i in _events.events)
		{
			if (approxEqual(i[1], noteStrum, 3) && (eventTag == i[3] || noteData == i[2] || eventValue == i[4]))
			{
				return;
			}
		}
		...
		_events.events.push([section, noteStrum, noteData, eventTag, eventValue]);
	...
	function deleteEvent(event:EventSprite):Void
	{
		...
		for (i in _events.events)
		{
			if (approxEqual(i[1], strumTime, 3) && tag == i[3] && value == i[4])
			{
	...
	function copyEventSection(?sectionNum:Int = 1)
	{
		...
		for (event in _events.events)
		{
			if (event[0] == daSec - sectionNum)
			{
				...
				var copiedNote:Array<Dynamic> = [daSec, strum, event[2], event[3], event[4]];
